...
这是一个用于在 Chromium 系浏览器（Chrome / Edge / Brave 等）中快速切换本地代理（SOCKS5 / HTTP）的扩展。

## 如何安装（开发者模式）

1. 打开浏览器扩展页：在地址栏输入 `chrome://extensions` 并回车
2. 打开右上角「开发者模式」
3. 点击「加载已解压的扩展程序」
4. 选择本项目目录（或 Release 下载的 zip 解压后的目录），确保目录内直接包含 `manifest.json`
...
1. 打包扩展为 zip（解压后根目录即 `manifest.json`）
2. 生成安全签名（Attestation）
3. 创建 Release 并上传 zip 产物
